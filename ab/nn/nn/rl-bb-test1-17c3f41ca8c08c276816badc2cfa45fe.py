...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
                ...
                self.backbone_a = TorchVision("regnet_x_800mf", in_channels=self._input_spec[0])
                self.backbone_b = TorchVision("shufflenet_v2_x2_0", in_channels=self._input_spec[0])
                self.features = nn.Sequential(FractalUnit(in_channels=self._input_spec[0], out_channels=64, num_columns=2, loc_drop_prob=0.1, dropout_prob=0.3), nn.MaxPool2d(kernel_size=2, stride=2))
