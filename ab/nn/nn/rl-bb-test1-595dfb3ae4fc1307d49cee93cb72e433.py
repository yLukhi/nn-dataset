...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
                ...
                self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=in_shape[1] if in_shape[1] == 3 else in_shape[1])
                self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=in_shape[1] if in_shape[1] == 3 else in_shape[1])
                self.features = nn.Sequential(FractalUnit(in_channels=in_shape[1] if in_shape[1] == 3 else in_shape[1], out_channels=64, num_columns=2, loc_drop_prob=0.15, dropout_prob=0.3))
                self._input_spec = (in_shape[1] if in_shape[1] == 3 else in_shape[1], in_shape[2], in_shape[3])
