...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision(model='regnet_y_800mf', in_channels=self._input_spec[0])
            self.backbone_b = TorchVision(model='shufflenet_v2_x1_0', in_channels=self._input_spec[0])
            self.features = nn.Sequential()
            curr_ch = self._input_spec[0]
            channels = [64 * (2 ** i) for i in range(2)]
            for i, out_ch in enumerate(channels):
                self.features.add_module(f"unit{i+1}", FractalUnit(curr_ch, out_ch, 2, 0.1, 0.1))
                curr_ch = out_ch
