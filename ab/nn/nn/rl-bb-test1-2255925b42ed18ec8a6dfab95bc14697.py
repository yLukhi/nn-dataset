...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision("regnet_x_1_6gf", in_channels=in_shape[1] if in_shape[1] == 3 else 3)
            ...
            for i, out_ch in enumerate(channels):
                self.features.add_module(f"unit{i+1}", FractalUnit(curr_ch, out_ch, 2, 0.15, 0.1))
                curr_ch = out_ch
