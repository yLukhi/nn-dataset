...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision(model='regnet_x_800mf', in_channels=c_in)
            self.backbone_b = TorchVision(model='shufflenet_v2_x1_0', in_channels=c_in)
            self.features = nn.Sequential(FractalUnit(in_channels=c_in, out_channels=64, num_columns=2, loc_drop_prob=0.1, dropout_prob=0.3), nn.MaxPool2d(kernel_size=2, stride=2))
