...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self._input_spec = (in_shape[1], in_shape[2], in_shape[3]) if in_shape[1] == 3 else (in_shape[1], in_shape[2], in_shape[3])
            self.pattern = "Parallel_Triple"
            self.backbone_a = TorchVision(model='resnet18', in_channels=self._input_spec[0])
            self.backbone_b = TorchVision(model='efficientnet_b0', in_channels=self._input_spec[0])
            self.features = nn.Sequential()
            curr_ch = self._input_spec[0]
            channels = [64 * (2 ** i) for i in range(1)]
            for i, out_ch in enumerate(channels):
                self.features.add_module(f"unit{i+1}", FractalUnit(curr_ch, out_ch, num_columns=2, loc_drop_prob=0.15, dropout_prob=0.3))
                curr_ch = out_ch
