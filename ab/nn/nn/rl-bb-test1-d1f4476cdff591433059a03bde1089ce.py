...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=c_in).to(device)
            self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=c_in).to(device)
            self.features = nn.Sequential(FractalUnit(in_channels=c_in, out_channels=64, num_columns=2, loc_drop_prob=0.15, dropout_prob=0.3)).to(device)
