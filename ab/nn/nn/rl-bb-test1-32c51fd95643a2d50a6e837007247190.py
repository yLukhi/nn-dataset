...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=in_shape[1] if in_shape[1] == 3 else 3)
            self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=in_shape[1] if in_shape[1] == 3 else 3)
            self.features = nn.Sequential()
            curr_ch = in_shape[1] if in_shape[1] == 3 else 3
            channels = [64 * (2 ** i) for i in range(1)]
            for i, out_ch in enumerate(channels):
                self.features.add_module(f"unit{i+1}", FractalUnit(curr_ch, out_ch, num_columns=2, loc_drop_prob=0.15, dropout_prob=0.1))
                curr_ch = out_ch
