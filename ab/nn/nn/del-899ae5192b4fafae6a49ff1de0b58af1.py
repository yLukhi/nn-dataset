...
class BagNetBottleneck(nn.Module):
    ...
    def conv1x1_block(self, in_channels, out_channels, activation=True):
        ...
        self.channels = [[64, 64, 124], [128, 128, 504], [256, 256, 293], [512, 512, 304]]
    ...
    def forward(self, x):
        nn.Conv2d(64, 195, kernel_size=1, stride=1, padding=0, bias=True),
        nn.ReLU(inplace=True),
        nn.Conv2d(195, 64, kernel_size=1, stride=1, padding=0, bias=True),
        nn.ReLU(inplace=True),
        nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        nn.Conv2d(128, 195, kernel_size=1, stride=1, padding=0, bias=True),
        nn.ReLU(inplace=True),
        nn.Conv2d(195, 128, kernel_size=1, stride=1, padding=0, bias=True),
        nn.ReLU(inplace=True),
        nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        return x
