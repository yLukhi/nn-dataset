...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision(model='regnet_x_800mf', in_channels=in_shape[1])
            self.backbone_b = TorchVision(model='shufflenet_v2_x2_0', in_channels=in_shape[1])
            self.backbone_c = TorchVision(model='regnet_x_1_6gf', in_channels=in_shape[1])
            self.features = FractalUnit(in_channels=in_shape[1], out_channels=64, num_columns=2, loc_drop_prob=0.15, dropout_prob=0.3)
            self._input_spec = (in_shape[1], in_shape[-2], in_shape[-1])
    ...
    def forward(self, x: torch.Tensor, is_probing: bool = False) -> torch.Tensor:
            ...
            x_c = adaptive_pool_flatten(self.backbone_c(x))
            fused = torch.cat([x_f, x_a, x_b, x_c], dim=1)
