...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=in_shape[1] if len(in_shape) == 4 else in_shape[0]).to(device)
            self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=in_shape[1] if len(in_shape) == 4 else in_shape[0]).to(device)
            self._input_spec = (in_shape[1] if len(in_shape) == 4 else in_shape[0], in_shape[-2], in_shape[-1])
    ...
    def forward(self, x: torch.Tensor, is_probing: bool = False) -> torch.Tensor:
            x_f = adaptive_pool_flatten(self.backbone_a(x)).flatten(1)
            x_a = adaptive_pool_flatten(self.backbone_b(x)).flatten(1)
