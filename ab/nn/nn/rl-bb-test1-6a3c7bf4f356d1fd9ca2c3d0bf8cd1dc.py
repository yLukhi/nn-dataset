...
def drop_conv3x3_block(in_channels, out_channels, stride=1, padding=1, bias=False, dropout_prob=0.0):
    conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias=bias)
    drop = nn.Dropout2d(p=dropout_prob) if dropout_prob > 0 else nn.Identity()
    return nn.Sequential(conv, nn.ReLU(inplace=True), drop)
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        dropout_prob = prm.get("dropout", 0.1)
        self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=self._input_spec[0])
        self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=self._input_spec[0])
        self.features = nn.Sequential(FractalUnit(in_channels=self._input_spec[0], out_channels=64, num_columns=2, loc_drop_prob=0.15, dropout_prob=dropout_prob))
