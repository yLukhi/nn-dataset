...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            super().__init__()
            self.device = device
            self._input_spec = in_shape[1:]
            self.use_amp = prm.get("use_amp", False)
            self.pattern = "Parallel_Triple"
            self.backbone_a = TorchVision("squeezenet1_0", in_channels=in_shape[1] if in_shape[1] == 3 else 3)
            self.backbone_b = TorchVision("regnet_x_1_6gf", in_channels=in_shape[1] if in_shape[1] == 3 else 3)
