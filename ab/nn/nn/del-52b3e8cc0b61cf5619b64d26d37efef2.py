...
class DPN131(nn.Module):
    def __init__(self, in_channels=3, num_classes=10, num_blocks=3, growth_rate=32):
        ...
        self.blocks = nn.ModuleList() ; self.add_module('0', self.blocks)
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x) ; self.add_module(str(sum([isinstance(n, nn.BatchNorm2d) for n in self.children()])), self.bn1)
