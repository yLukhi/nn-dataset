...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
                ...
                self.backbone_a = TorchVision("regnet_x_800mf", in_channels=self._input_spec[0]).to(device)
                self.backbone_b = TorchVision("squeezenet1_0", in_channels=self._input_spec[0]).to(device)
                self.features = nn.Sequential(FractalUnit(in_channels=self._input_spec[0], out_channels=64, num_columns=2, loc_drop_prob=0.15, dropout_prob=0.3))
