...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self.backbone_a = TorchVision("regnet_x_1_6gf", in_channels=in_channels)
            self.backbone_b = TorchVision("squeezenet1_0", in_channels=in_channels)
            self.features = nn.Sequential(FractalUnit(in_channels=in_channels, out_channels=64, num_columns=2, loc_drop_prob=0.15, dropout_prob=dropout_prob), nn.MaxPool2d(kernel_size=2, stride=2))
