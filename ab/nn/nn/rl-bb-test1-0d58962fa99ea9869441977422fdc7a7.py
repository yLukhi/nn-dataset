...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            if len(in_shape) == 4:
                c_in, h, w = in_shape[1], in_shape[2], in_shape[3]
            else:
                c_in, h, w = in_shape[0], in_shape[1], in_shape[2]
            self._input_spec = (c_in, h, w)
            # --- Filled by Generator ---
            self.backbone_a = TorchVision("resnet34", in_channels=c_in).to(device)
            self.backbone_b = TorchVision("shufflenet_v2_x1_0", in_channels=c_in).to(device)

            self.features = nn.Sequential()
            curr_ch = c_in
            channels = [64 * (2 ** i) for i in range(1)]
            for i, out_ch in enumerate(channels):
                self.features.add_module(f"unit{i+1}", FractalUnit(curr_ch, out_ch, 2, 0.1, dropout_prob))
                curr_ch = out_ch
