...
class DPNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv1 = nn.Conv2d(
            in_channels, out_channels, kernel_size=5, stride=stride, padding=2, groups=4, bias=False
        )
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(
            out_channels, out_channels, kernel_size=5, padding=2, groups=4, bias=False
        )
...
class DPN107(nn.Module):
    def __init__(self, in_channels, num_classes, num_blocks, growth_rate):
        ...
        self.features = nn.Sequential(nn.Linear(7*7*growth_rate, 4096), nn.ReLU(True), nn.Linear(4096, 11))
