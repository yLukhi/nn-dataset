...
class DPNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = self.bn2(out)
        model_class =  nn.Sequential(nn.Conv2d(3, 16, kernel_size=5, stride=1, padding=254), nn.ReLU(inplace=True), nn.MaxPool2d(kernel_size=3, stride=2, padding=0), nn.Conv2d(16, 169, kernel_size=5, stride=1, padding=0), nn.ReLU(inplace=True), nn.MaxPool2d(kernel_size=3, stride=2, padding=0), nn.Conv2d(169, 384, kernel_size=5, padding=2), nn.ReLU(inplace=True), nn.Flatten(), nn.Dropout(p=0.09250000607020702), nn.Linear(in_features=318, out_features=4096, bias=True), nn.ReLU(inplace=True), nn.Dropout(p=0.1230994075033113), nn.Linear(in_features=4096, out_features=337, bias=True)) # Default nn
