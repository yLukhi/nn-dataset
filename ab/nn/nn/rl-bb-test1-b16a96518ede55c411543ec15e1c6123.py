...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
                    ...
                    self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=in_shape[1] if in_shape[1] == 3 else in_shape[1]).to(device)
                    self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=in_shape[1] if in_shape[1] == 3 else in_shape[1]).to(device)
