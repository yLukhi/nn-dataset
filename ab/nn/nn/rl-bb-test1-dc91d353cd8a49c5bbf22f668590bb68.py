...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
            ...
            self._input_spec = (c_in, in_shape[-2], in_shape[-1])
            self.pattern = "Parallel_Triple"
            self.backbone_a = TorchVision(model='regnet_x_3_2gf', in_channels=c_in)
            self.backbone_b = TorchVision(model='squeezenet1_0', in_channels=c_in)
            self.features = nn.Sequential(
                FractalUnit(in_channels=c_in, out_channels=64, num_columns=2, loc_drop_prob=0.1, dropout_prob=0.3),
                FractalUnit(in_channels=64, out_channels=128, num_columns=2, loc_drop_prob=0.1, dropout_prob=0.3)
            )
